// Server-side BIN database with lazy loading
...
async function loadDatabase(brand: string): Promise<BinDatabase> {
  ...
  const loadPromise = (async () => {
    try {
      const filePath = join(process.cwd(), 'data', 'bin', `${brand}.json`);
...
export async function getBinInfo(bin: string): Promise<BinInfo> {
  ...
  return {
    brand: brandName === 'Others' ? 'Unknown' : brandName,
    type: 'Unknown',
    level: 'Standard',
    bank: 'Unknown Bank',
    country: 'Unknown'
